...
st.caption("Version 3.7 live Streamlit interface with scenario saving, editable inputs, and trade studies.")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    ["Performance", "Operating points", "Mission", "Scenario comparison", "Config", "Trade study"]
)
...
with tab3:
    st.subheader("Mission")
    if mission_profile is not None:
        st.dataframe(mission_profile_segments_df(mission_profile).round(3), use_container_width=True)
    else:
...
with tab6:
    ...
    trade_parameter = st.selectbox(
        "Parameter to sweep",
        options=["battery_mass_kg", "payload_mass_kg", "wing_area_m2", "cd0", "altitude_m", "cruise_speed_m_per_s"],
    )
