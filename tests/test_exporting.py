...
def test_save_sweep_to_csv_creates_file(tmp_path: Path):
    config = make_test_config()
    sweep_df = build_speed_sweep(config, max_speed_m_per_s=40.0, num_points=50)

    output = tmp_path / "speed_sweep_results.csv"
    returned_path = save_sweep_to_csv(sweep_df, output)

    assert returned_path.exists()
    assert returned_path == output


def test_save_comparison_to_csv_creates_file(tmp_path: Path):
    ...
    output = tmp_path / "configuration_comparison_results.csv"
    returned_path = save_comparison_to_csv(comparison_df, output)
